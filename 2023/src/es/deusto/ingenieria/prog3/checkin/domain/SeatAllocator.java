...
public class SeatAllocator {
	...
	public Map<Integer, List<Seat>> initRowsMap(List<Seat> seats) {
		...
		for (Seat s : seats) {
			if (!seatsByRow.containsKey(s.getRow())) {
				seatsByRow.put(s.getRow(), new ArrayList<Seat>());
			}
	...
	// dos asientos del pasillo (C y D), en ese caso, guardas dos grupos de
	// asientos:
	...
	public List<List<Seat>> findAdjacentGroups(SeatClass seatClass, int number, Map<Integer, List<Seat>> seatsMap) {
		...
		for (Integer row : seatsMap.keySet()) {
			adjacentGroup = new ArrayList<Seat>();
	...
	public void orderAdjacentGroups(List<List<Seat>> adjacentGroups) {
		Comparator<List<Seat>> seatComparator = new Comparator<List<Seat>>() {
			@Override
			public int compare(List<Seat> l1, List<Seat> l2) {
